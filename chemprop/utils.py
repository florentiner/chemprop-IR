...
def load_checkpoint(path: str,
                    ...
                    additional_args: Namespace = None,
                    cuda: bool = None,
    ...
    # Load spectral mask
    if 'spectral_mask' in state.keys():
        model.spectral_mask=state['spectral_mask']

    return model


def load_frzn_mpn(model: torch.nn,
                    path: str,
                    current_args: Namespace = None,
                    cuda: bool = None,
                    logger: logging.Logger = None) -> MoleculeModel:
    ...
    # Overwrite MPN parameters and skip missing parameters and parameters of mismatched size
    for param_name in ['encoder.encoder.W_i.weight', 'encoder.encoder.W_h.weight', 'encoder.encoder.W_o.weight', 'encoder.encoder.W_o.bias']:
        if param_name not in model_state_dict:
            debug(f'Pretrained parameter "{param_name}" cannot be found in model parameters.')
        elif model_state_dict[param_name].shape != loaded_state_dict[param_name].shape:
            debug(f'Pretrained parameter "{param_name}" '
...
def get_loss_func(args: Namespace) -> nn.Module:
    ...
    if args.dataset_type == 'spectra':
        if args.spectral_loss_function == 'wasserstein':
            return wasserstein
        elif args.spectral_loss_function == 'jsd': #Jensen-Shannon Divergence
            return jsd
        elif args.spectral_loss_function == 'stmse':
            return stmse
        elif args.spectral_loss_function == 'srmse':
            return srmse
        elif args.spectral_loss_function == 'smse':
            return smse
        else:
            return sid #Spectral information divergence
